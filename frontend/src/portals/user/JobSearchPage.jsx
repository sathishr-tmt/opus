...
function JobSearchPage({ onDashboardChange, showToast }) {
  ...
  // Tailor the resume for one job.
  //
  // Saving is a precondition on the backend, so an unsaved job is saved here
  // first rather than returning an error the user has to act on themselves.
  async function tailorResume(job) {
    const isSaved = savedJobIds.includes(String(job.id));

    if (job.atsScore != null && job.atsScore < 40) {
      const proceed = window.confirm(
        `This role scores ${job.atsScore}%.\n\n` +
          `The missing skills are core requirements for it, and a rewrite cannot ` +
          `add skills you have not listed.\n\nGenerate it anyway?`
      );
      if (!proceed) return;
    }
    ...
    try {
      if (!isSaved) {
        const saved = await apiRequest(`/api/saved/${job.id}`, { method: 'POST' });
      ...
      {selectedJob && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-slate-950/60 p-6 backdrop-blur-sm">
          <div className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-xl border border-slate-200 bg-white p-5 shadow-2xl">
            ...
            {/* Description on the left, match analysis on the right. */}
            <div className="mt-5 grid gap-4 lg:grid-cols-[1.4fr_1fr]">
              ...
              <MatchPanel
                ...
                showToast={showToast}
                rewriting={tailoring}
                onRewrite={() => tailorResume(selectedJob)}
                saved={savedJobIds.includes(String(selectedJob.id))}
              />
            </div>
